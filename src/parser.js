...
async function main() {
    ...
    for (let i = 0; i < token.length; i++) {
        ...
        // Make it so you can add a variable where there is the maths.
        if (token[i][0] === "calc") {
            let connectedString = "";
            let name = "";
        ...
        if (token[i][0] === "wait") {
            if (token[i][1] !== "") {
                if (token[i][1].isNumber()) {
                    ...
                    await delay(`${Number(token[i][1])}000`);
                } else if (token[i][1].isVariable()) {
                    const delay = ms => new Promise(resolve => setTimeout(resolve, ms));
                    await delay(`${variable[token[i][1]]}000`);
                }
            }
